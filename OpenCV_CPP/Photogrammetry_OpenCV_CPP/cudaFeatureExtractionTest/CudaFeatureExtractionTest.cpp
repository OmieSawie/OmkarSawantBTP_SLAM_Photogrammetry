    ...
    void extractFeatures_goodFeaturesToTrack(int, void *) {
        ...
        if (!prev_src_gray_gpu.empty()) {
            ...
            cv::Mat fundamentalMatrix = cv::findFundamentalMat(
                points1, points2, cv::FM_RANSAC, 3, 0.95);
            /*  cout << fundamentalMatrix << endl; */

            // Estimating the Essential Matrix
            cv::Mat cameraMatrix = (cv::Mat_<float>(3, 3) << 2422, 0, 2138, 0,
                                    2430, 1137, 0, 0, 1);
...
int main() {
    ...
    src_gray = cv::imread("../testimg1.jpg", cv::IMREAD_GRAYSCALE);
    prev_src_gray = cv::imread("../testimg2.jpg", cv::IMREAD_GRAYSCALE);
