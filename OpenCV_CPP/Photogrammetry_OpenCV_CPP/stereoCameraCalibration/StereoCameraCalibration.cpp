...
static int print_help(char **argv) {
    cout << " Given a list of chessboard or ChArUco images, the number of "
            "corners (nx, ny)\n"
            " on the chessboards and the number of squares (nx, ny) on "
            "ChArUco,\n"
    ...
    cout << "Usage:\n " << argv[0]
         ...
         << " -t=<pattern type: chessboard or charucoboard default=chessboard> "
            "-s=<square_size default=1.0> -ms=<marker size default=0.5>"
         << " -ad=<predefined aruco dictionary name default=DICT_4X4_50> "
            "-adf=<aruco dictionary file default=None>"
         << " <image list XML/YML file default=stereo_calib.xml>\n"
         << endl;
    cout << "Available Aruco dictionaries: DICT_4X4_50, DICT_4X4_100, "
            "DICT_4X4_250, "
...
static void StereoCalib(const vector<string> &imagelist, Size inputBoardSize,
                        string type, float squareSize, float markerSize,
                        cv::aruco::PredefinedDictionaryType arucoDict,
                        string arucoDictFile, bool displayCorners = false,
                        bool useCalibrated = true, bool showRectified = true) {
    if (imagelist.size() % 2 != 0) {
        cout << "Error: the image list contains odd (non-even) number of "
                "elements\n";
        return;
    }
    ...
    cv::aruco::CharucoBoard ch_board(boardSizeUnits, squareSize, markerSize,
                                     dictionary);
    ...
    for (i = j = 0; i < nimages; i++) {
        for (k = 0; k < 2; k++) {
            ...
            for (int scale = 1; scale <= maxScale; scale++) {
                ...
                if (type == "chessboard") {
                    found = findChessboardCorners(
                        timg, boardSizeInnerCorners, corners,
                        CALIB_CB_ADAPTIVE_THRESH | CALIB_CB_NORMALIZE_IMAGE);
                } else if (type == "charucoboard") {
                    ch_detector.detectBoard(timg, corners, markerIds);
                    found = corners.size() ==
                            (size_t)(boardSizeInnerCorners.height *
                                     boardSizeInnerCorners.width);
                } else {
            ...
            if (displayCorners) {
                ...
                drawChessboardCorners(cimg, boardSizeInnerCorners, corners,
                                      found);
    ...
    // from the fundamental matrix
    {
        ...
        F = findFundamentalMat(Mat(allimgpt[0]), Mat(allimgpt[1]), FM_8POINT, 0,
                               0);
        Mat H1, H2;
    ...
    for (i = 0; i < nimages; i++) {
        for (k = 0; k < 2; k++) {
            Mat img = imread(goodImageList[i * 2 + k], IMREAD_GRAYSCALE), rimg,
                cimg;
...
int main(int argc, char **argv) {
    ...
    cv::CommandLineParser parser(
        argc, argv,
        "{w|9|}{h|6|}{t|chessboard|}{s|1.0|}{ms|0.5|}{ad|DICT_4X4_50|}{adf|"
        "None|}{nr||}{help||}{@input|stereo_calib.xml|}");
    if (parser.has("help"))
    ...
    cv::aruco::PredefinedDictionaryType arucoDict;
    if (arucoDictName == "DICT_4X4_50") {
        arucoDict = cv::aruco::DICT_4X4_50;
    } else if (arucoDictName == "DICT_4X4_100") {
        arucoDict = cv::aruco::DICT_4X4_100;
    } else if (arucoDictName == "DICT_4X4_250") {
        arucoDict = cv::aruco::DICT_4X4_250;
    } else if (arucoDictName == "DICT_4X4_1000") {
        arucoDict = cv::aruco::DICT_4X4_1000;
    } else if (arucoDictName == "DICT_5X5_50") {
        arucoDict = cv::aruco::DICT_5X5_50;
    } else if (arucoDictName == "DICT_5X5_100") {
        arucoDict = cv::aruco::DICT_5X5_100;
    } else if (arucoDictName == "DICT_5X5_250") {
        arucoDict = cv::aruco::DICT_5X5_250;
    } else if (arucoDictName == "DICT_5X5_1000") {
        arucoDict = cv::aruco::DICT_5X5_1000;
    } else if (arucoDictName == "DICT_6X6_50") {
        arucoDict = cv::aruco::DICT_6X6_50;
    } else if (arucoDictName == "DICT_6X6_100") {
        arucoDict = cv::aruco::DICT_6X6_100;
    } else if (arucoDictName == "DICT_6X6_250") {
        arucoDict = cv::aruco::DICT_6X6_250;
    } else if (arucoDictName == "DICT_6X6_1000") {
        arucoDict = cv::aruco::DICT_6X6_1000;
    } else if (arucoDictName == "DICT_7X7_50") {
        arucoDict = cv::aruco::DICT_7X7_50;
    } else if (arucoDictName == "DICT_7X7_100") {
        arucoDict = cv::aruco::DICT_7X7_100;
    } else if (arucoDictName == "DICT_7X7_250") {
        arucoDict = cv::aruco::DICT_7X7_250;
    } else if (arucoDictName == "DICT_7X7_1000") {
        arucoDict = cv::aruco::DICT_7X7_1000;
    } else if (arucoDictName == "DICT_ARUCO_ORIGINAL") {
        arucoDict = cv::aruco::DICT_ARUCO_ORIGINAL;
    } else if (arucoDictName == "DICT_APRILTAG_16h5") {
        arucoDict = cv::aruco::DICT_APRILTAG_16h5;
    } else if (arucoDictName == "DICT_APRILTAG_25h9") {
        arucoDict = cv::aruco::DICT_APRILTAG_25h9;
    } else if (arucoDictName == "DICT_APRILTAG_36h10") {
        arucoDict = cv::aruco::DICT_APRILTAG_36h10;
    } else if (arucoDictName == "DICT_APRILTAG_36h11") {
    ...
    StereoCalib(imagelist, inputBoardSize, type, squareSize, markerSize,
                arucoDict, arucoDictFile, false, true, showRectified);
    return 0;
}
