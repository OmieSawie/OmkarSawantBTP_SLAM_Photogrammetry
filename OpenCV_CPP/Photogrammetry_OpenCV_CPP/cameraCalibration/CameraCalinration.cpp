...
int main(int argc, char *argv[]) {
    ...
    for (;;) {
        ...
        if (mode == CAPTURING) {
            if (s.showUndistorted)
                msg = cv::format("%d/%d Undist", (int)imagePoints.size(),
                                 s.nrFrames);
            else
        ...
        if (mode == CALIBRATED && s.showUndistorted) {
            ...
            if (s.useFisheye) {
                ...
                fisheye::estimateNewCameraMatrixForUndistortRectify(
                    cameraMatrix, distCoeffs, imageSize, Matx33d::eye(),
                    newCamMat, 1);
    ...
    if (s.inputType == Settings::IMAGE_LIST && s.showUndistorted &&
        ...
        if (s.useFisheye) {
            Mat newCamMat;
            fisheye::estimateNewCameraMatrixForUndistortRectify(
                cameraMatrix, distCoeffs, imageSize, Matx33d::eye(), newCamMat,
                1);
            fisheye::initUndistortRectifyMap(cameraMatrix, distCoeffs,
                                             Matx33d::eye(), newCamMat,
...
static void saveCameraParams(Settings &s, Size &imageSize, Mat &cameraMatrix,
    ...
    if (s.writeExtrinsics && !rvecs.empty() && !tvecs.empty()) {
        ...
        fs.writeComment("a set of 6-tuples (rotation vector + translation "
